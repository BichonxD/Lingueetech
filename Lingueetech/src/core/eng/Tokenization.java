...
public class Tokenization {
	...
	private ArrayList<String> indexIdToLemme; // probablement non n√©cessaire car la perte de place est surement trop grande pour le gain de temps trop petit
	...
	public Tokenization(){
		...
		indexIdToLemme = new ArrayList<String>(); // probablement non n√©cessaire car la perte de place est surement trop grande pour le gain de temps trop petit
	...
	/**
	 * Analyser un fichier et enregistrer les √©l√©ments obtenus dans les structures
		...
		// On traite chaque phrase dans le fichier separament
				...
				for (CoreMap sentence : sentences) {
					...
					while (iterator.hasNext()) {
						...
						// On ne traite pas les numÔøΩros et les symboles
						if (isLemma(lemma)) {
							...
							if (!indexIdToLemme.contains(lemma)) {
								...
								itemp = -1;
								// m√ j de l'arrayList du lemme existant avec la sentence actuelle
								...
								//pr√©caution que nous prenons : on s'assure que le lemme est bien dans notre hash, si ce n'est pas le cas probl√®me !
								if (itemp == -1)
									System.out.println("c'est la merde les gars !");
								//on peut ajouter notre id de phrase √  la liste de phrases du lemme
	 ...
	 * @return Le tableau de suffix du mot (la position de dÈbut est 0)
	 */
